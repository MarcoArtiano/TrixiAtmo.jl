...
struct CompressibleMoistEulerPotentialTemperatureEquations2D{RealT <: Real} <:
       AbstractCompressibleMoistEulerPotentialTemperatureEquations{2, 6}
...
@inline function flux_shima_etal(u_ll, u_rr, orientation, equations::CompressibleMoistEulerPotentialTemperatureEquations2D)
  ...
  @inline function flux_theta(u_ll, u_rr, orientation, equations::CompressibleMoistEulerPotentialTemperatureEquations2D)
    # Unpack left and right state
    rho_ll, rho_v1_ll, rho_v2_ll, rho_theta_ll, rho_qv_ll, rho_qc_ll = u_ll
    rho_rr, rho_v1_rr, rho_v2_rr, rho_theta_rr, rho_qv_rr, rho_qc_rr = u_rr
    ...
    #rho_qv_mean = stolarsky_mean(rho_qv_ll, rho_qv_rr, equations.gamma)
    rho_qv_mean = ln_mean(rho_qv_ll, rho_qv_rr)
    end
    rho_qc_mean = 0.0
    if (!(rho_qc_ll == 0.0) && !(rho_qc_rr == 0.0))
    #rho_qc_mean = stolarsky_mean(rho_qc_ll, rho_qc_rr, equations.gamma)
    rho_qc_mean = ln_mean(rho_qc_ll, rho_qc_rr)
    end
    ...
    if orientation == 1
      ...
      f3 = f1 * v2_avg
      f4 = gammamean * v1_avg  
      f5 = rho_qv_mean * v1_avg 
      f6 = rho_qc_mean * v1_avg
    else
      f1 = rho_avg * v2_avg
      f2 = f1 * v1_avg
      f3 = f1 * v2_avg + p_avg
      f4 = gammamean * v2_avg 
      f5 = rho_qv_mean * v2_avg 
      f6 = rho_qc_mean * v2_avg
    end
...
@inline function cons2entropy(u, equations::CompressibleMoistEulerPotentialTemperatureEquations2D)
  ...
  v_square = v1^2 + v2^2
  rho_d = rho - rho_qv - rho_qc
  kappa_M = (equations.R_d * rho_d + equations.R_v * rho_qv) / (equations.c_pd * rho_d + equations.c_pv * rho_qv +
             equations.c_pl * rho_qc)
  p = (equations.R_d * rho_theta / (equations.p_0.^kappa_M)).^(1/(1-kappa_M))
...
@inline function cons2aeqpot(cons, equations::CompressibleMoistEulerPotentialTemperatureEquations2D)
    @unpack c_pd, c_pv, c_pl, R_d, R_v, p_0, L_00 = equations
    rho, rho_v1, rho_v2, rho_theta, rho_qv, rho_ql = cons
